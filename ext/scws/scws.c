...
static VALUE
rb_scws_init(VALUE self, VALUE text){
	struct rb_scws_malloc *ptr;
...
static VALUE
rb_scws_set_dic(VALUE self, VALUE r_path){
	Check_Type(r_path, T_STRING);
	char* path = RSTRING_PTR(r_path);
	struct rb_scws_malloc *ptr;
	Data_Get_Struct(self, struct rb_scws_malloc, ptr);
	scws_add_dict(ptr->engine, path, 1);
}

static VALUE
rb_scws_split(VALUE self, VALUE r_text){
	...
	while (res = cur = scws_get_result(ptr->engine))
  {
    while (cur != NULL)
    {
			...
			// char str2[cur->len+1];
			// strncpy(str2, text + cur->off, cur->len);
			// str2[cur->len]='\0';
			// rb_hash_aset(item, rb_str_new2("text"), rb_str_export_to_enc(rb_str_new2(str2), encoding));
...
void Init_scws(void){
  ...
  cScws = rb_const_get(rb_cObject, rb_intern("Scws"));
  ...
  rb_define_method(cScws, "add_dic", rb_scws_set_dic, 1);
  rb_define_method(cScws, "split", rb_scws_split, 1);
}
